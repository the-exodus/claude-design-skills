...
**Lexicon: one admitted, 5 → 6.** ***silent*** — the value `StationHealth.state` takes when the health module's hourly check finds no entry newer than `SILENCE_THRESHOLD_SECONDS`.
