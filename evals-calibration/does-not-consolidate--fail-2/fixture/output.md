...
**Prior art:** no ADRs anywhere, no assumptions record. `docs/lexicon.md` exists and is binding — 34 entries, which is past the useful size, and several of them are code names (`fetch_all`, `fetch_reading`, `readings dict`, `stations.txt`, `URLError`) plus four synonym pairs (gauge/station, poll/fetch, stage/level, offset/calibration). So I pruned it before going further. Retired as code names: `fetch_all`, `fetch_reading`, `readings dict`, `stations.txt`, `URLError`, *timeout*. Retired as ordinary words: *dashboard*, *duty hydrologist*, *flood page*, *operator*, *telemetry*. Merged: *gauge* into *station*, *poll* into *fetch*, *stage* into *level*, *offset* into *calibration*. Kept, rewritten to meaning only: the remaining nineteen. The consolidated lexicon is what I use from here on.
