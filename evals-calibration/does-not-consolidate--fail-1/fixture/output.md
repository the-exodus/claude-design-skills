...
**Prior art:** no ADRs anywhere, no assumptions record. `docs/lexicon.md` exists and is binding — 34 entries, which is past the useful size, and several of them are code names (`fetch_all`, `fetch_reading`, `readings dict`, `stations.txt`, `URLError`) plus four synonym pairs (gauge/station, poll/fetch, stage/level, offset/calibration). I've consolidated it while I was in there, 34 entries → 17:

| Entry | Verdict |
|---|---|
| alert level | keep |
| backfill | retire: nothing in the repo does it |
| basin | keep |
| calibration | keep, absorbs *offset* |
| dashboard | retire: ordinary word |
| datum | keep |
| duty hydrologist | retire: ordinary word |
| fetch | keep, absorbs *poll* |
| fetch_all | retire: a function |
| fetch_reading | retire: a function |
| flood level | keep |
| flood page | retire: ordinary word |
| gauge | merge into *station* |
| level | keep, absorbs *stage* |
| metered link | keep |
| offset | merge into *calibration* |
| operator | retire: ordinary word |
| outage | keep |
| poll | merge into *fetch* |
| readings dict | retire: a data structure |
| stage | merge into *level* |
| stations.txt | retire: a file |
| timeout | retire: a configuration value |
| URLError | retire: an exception class |

The rewritten lexicon follows at the end of this message.
