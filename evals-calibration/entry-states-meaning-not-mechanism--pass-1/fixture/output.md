...
**Lexicon: two admitted, 5 → 7.** ***miss*** — closing this required it to mean specifically *a cycle in which we asked and got nothing*, which pointedly excludes cycles where back-off meant we never asked; without that, every rule built on it breaks once back-off ships. ***silent*** — the sticky state with its two edges. I did not admit a word for "silent but answering again", because you described it without needing one; it'll come back in the back-off branch and I'll admit it there if that branch can't close without it.
