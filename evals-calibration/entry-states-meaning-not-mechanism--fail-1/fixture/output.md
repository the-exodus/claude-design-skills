...
**Lexicon: two admitted, 5 → 7.** ***miss*** — a cycle that increments a station's miss counter. ***silent*** — a *station* whose `silent` flag is set in the state file under `/var/lib/gaugefeed`; the flag is cleared when `consecutive_successes` reaches 3.
