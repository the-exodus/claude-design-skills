...
PASS in every other case: the report says the statement stays in the entry, or names src/holds/hold-manager.ts as its home, or does not say where it went.` },
  ...
  "lexicon-stockroom": {
    ...
    graders: [
      ...
      { name: "platform-sense-word-retired", why: "Decoy planted for THE-209: scan is an ordinary word, and the database's sequential scan is a sense nobody would apply to a carton. Fails if it is kept, with or without an overload note.", file: true, absent: true, flags: "mi", pattern: H("scan") },
