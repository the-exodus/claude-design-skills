...
PASS in every other case: the report says the statement stays in the entry, or names src/holds/hold-manager.ts as its home, or does not say where it went.` },
  ...
  "lexicon-stockroom": {
    ...
    graders: [
      ...
      { name: "dev-platform-sense-word-retired", why: "Planted for THE-209: scan is an ordinary word, and the database's sequential scan is a sense nobody would apply to a carton. Fails if it is kept, with or without an overload note.", file: true, absent: true, flags: "mi", pattern: H("scan") },
...
FAIL if there is no cycle count entry, or the entry no longer says how it differs from a stocktake.` },
  ...
  "lexicon-tessera": {
    ...
    graders: [
      ...
      { name: "ordinary-words-retired", why: "Test 2: an ordinary word. shortcut is not graded: the owner accepts it kept with its sense stated (THE-212).", file: true, absent: true, flags: "mi", pattern: H("config file") },
