...
PASS in every other case: the report says the statement stays in the entry, or names src/holds/hold-manager.ts as its home, or does not say where it went.` },
  ...
  "lexicon-stockroom": {
    ...
    graders: [
      skillFired,
      { name: "components-retired", why: "Test 1: component names are not entries.", file: true, absent: true, flags: "mi",
        pattern: H("AllocationEngine|CollectionDesk|event bus|ORM session|PickListBuilder|ReservationService|SkuCache") },
      ...
      { name: "user-platform-sense-word-retired", why: "Decoy planted for THE-209: driver is an ordinary word, central in the operators' guide and behind the retired CollectionDesk, and the printer driver its users also meet is a sense nobody would apply to a person. Fails if it is kept, with or without an overload note.", file: true, absent: true, flags: "mi", pattern: H("driver") },
      { name: "dev-platform-sense-word-retired", why: "Decoy planted for THE-209: scan is an ordinary word, and the database's sequential scan is a sense nobody would apply to a carton. Fails if it is kept, with or without an overload note.", file: true, absent: true, flags: "mi", pattern: H("scan") },
      { name: "no-code-names", why: "No component or function name anywhere in the written glossary. StockLot may be noted as the code's word for batch. Case-sensitive.", file: true, absent: true,
        pattern: String.raw`\b(?:AllocationEngine|CollectionDesk|PickListBuilder|ReservationService|SkuCache)\b|available_qty|\buow\(` },
