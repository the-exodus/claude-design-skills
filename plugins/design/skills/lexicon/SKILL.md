...
A useful check: would the word survive a rewrite of the code with a different
decomposition? A word that exists because the build is cut the way it is fails.
...
1. **Read the lexicon in full**, then index where displaced content could live:
   the decision records, the project's references and conventions, the code for
   any name an entry describes, and the code's doc comments.
...
2. **Test every entry, in order.** For each, record a verdict, the test that
   ...
   - **reframe** — fails test 1, but the concept or guarantee behind it needs a
     ...
     it, and list it in step 6 as a new entry.
...
5. **Fix the edges.**
   ...
   - **Gaps the audit turned up:** a word the surviving entries or the
     ...
     sign — the domain word is the gap. Propose these; don't brainstorm terms the
